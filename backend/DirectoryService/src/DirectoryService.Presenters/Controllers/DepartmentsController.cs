...
using DirectoryService.Application.Departments.UseCases.AttachVideo;
...
public class DepartmentsController : ControllerBase
{
    ...
    [ProducesResponseType<Envelope>(500)]
    [ProducesResponseType<Envelope>(409)]
    [EndpointSummary("Прикрепить видео к подразделению")]
    public async Task<EndpointResult<Guid>> AttachVideo(
        [FromRoute][Description("Идентификатор подразделения")] Guid id,
        [FromBody][Description("Данные для прикрепления")] AttachVideoForDepartmentRequest request,
        [FromServices] AttachVideoForDepartmentHandler handler,
        CancellationToken cancellationToken) =>
        await handler.Handle(new AttachVideoForDepartmentCommand(id, request), cancellationToken);
}